<template>
    ...
    >
        ...
        >
            ...
            >
                <template v-if="customer">
                    ...
                    <h3
                        class="tw-font-semibold tw-text-xl tw-truncate tw-capitalize"
                    >
                        {{ customer.name }}
                    </h3>
                    <p>
        ...
        <div class="tw-flex tw-items-center tw-gap-x-3">
            ...
            <div
                class="tw-flex tw-justify-center tw-items-center tw-bg-amber-400 tw-h-12 tw-w-1/3"
            >
                {{}}
            </div>
        </div>
...
const goToProductArea = () => {
    nextTick(() =>
        focusElement(`#input-${form.sales_items.length - 1}-quantity input`),
    );
};

const customer = ref(
    props.customers.find((item) => item.id === form.customer_id) || '',
);
...
form.sub_total = computed(() =>
    form.sales_items
        .reduce((acc, item) => acc + parseFloat(item.sub_total), 0)
        .toFixed(2),
);
form.grand_total = computed(() => {
    ...
    return (
        ...
        parseFloat(extra_fees) -
        parseFloat(discount)
    ).toFixed(2);
});
form.due_amount = computed(() =>
    (parseFloat(form.grand_total) - parseFloat(form.paid_amount)).toFixed(2),
);
