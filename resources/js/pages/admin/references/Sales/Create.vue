<template>
  ...
  <div
    ...
    <div
      ...
      <!-- First Column -->
      <div class="tw-space-y-2 tw-bg-amber-100/60 tw-p-2 tw-pt-3 tw-pr-5 sm:tw-w-1/3 tw-text-center sm:tw-text-left">
        <div class="tw-border-white/80 tw-bg-gray-100/30 tw-p-2 tw-border tw-font-semibold tw-leading-5">
          MEMO: {{ form.bill_no }}
        </div>
        <div class="tw-w-full">
          <div class="tw-flex tw-w-full">
            <ComboBox class="tw-grow" :persons="customers" v-model="form.customer" input-id="customer-select"
      ...
      <div class="sm:tw-text-right tw-bg-amber-100/60 tw-p-2 sm:tw-w-1/3 tw-text-center">
        <template v-if="customer">
          ...
          <h3 class="tw-font-semibold tw-text-xl tw-truncate">{{ customer.name }}</h3>
    ...
    <div class="tw-flex tw-items-center tw-gap-x-3">
      ...
      <div class="tw-flex tw-justify-center tw-items-center tw-bg-amber-300 tw-h-12 tw-w-1/3">
        STOCK: {{ }}
      </div>
    </div>
    ...
    <div class="tw-gap-0.5 tw-grid tw-grid-cols-[32px_1fr_90px_90px_90px_90px_30px]">
      ...
      <div
        ...
        <template v-for="(product, index) in form.sales_items" :key="product.id">
          ...
          <div class="tw-h-8 __tbody">
            <CurrencyInput classes="tw-input-sm tw-h-8" v-model="product.quantity" :id="`input-${index}-quantity`"
              @keydown.up.prevent="navigateTo(index - 1, 'quantity')"
              @keydown.down.prevent="navigateTo(index + 1, 'quantity')"
              @keydown.enter.prevent="navigateTo(index, 'rate')" @keydown.right.prevent="navigateTo(index, 'rate')" />
          </div>
          <div class="tw-h-8 __tbody">
            <CurrencyInput classes="tw-input-sm tw-h-8" v-model="product.unit_price" :id="`input-${index}-rate`"
              @keydown.up.prevent="navigateTo(index - 1, 'rate')" @keydown.down.prevent="navigateTo(index + 1, 'rate')"
          ...
          <div class="tw-h-8 __tbody">
            <CurrencyInput classes="tw-input-sm tw-h-8" v-model="product.discount" :id="`input-${index}-discount`"
              @keydown.up.prevent="navigateTo(index - 1, 'discount')"
      ...
      <div
        ...
        <div v-show="form.paid_amount > 0"
          ...
          <!-- Payment Methods -->
          <PaymentMethods class="tw-w-full" :payment_types="payment_types" v-model:paymentType="form.payment_type" />

          <div v-if="form.payment_type?.ref_name" class="tw-form-control">
            <label class="tw-mb-0 tw-pt-0 tw-label">
              <span class="tw-label-text tw-font-normal tw-text-sm">{{ form.payment_type?.ref_name }}</span>
            </label>
      ...
      <div class="tw-gap-0.5 tw-grid tw-grid-cols-subgrid tw-col-span-4 tw-row-span-6">
        ...
        <div class="__tfoot_sum_amount">
          <CurrencyInput classes="tw-h-8 tw-content-center tw-font-black" v-model="form.sub_total" :disabled="true" />
        </div>
        ...
        <div class="__tfoot_sum_amount">
          <CurrencyInput id="paid-amount" v-model.number="form.paid_amount"
            classes="tw-h-8 tw-content-center tw-font-black" @keydown.up.prevent="focusElement('#extra-fees input')"
...
const { navigateTo, addProductToInvoice, removeItem, goToProductArea } = useInvoiceForm(form, {
