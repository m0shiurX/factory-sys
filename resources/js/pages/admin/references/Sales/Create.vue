<template>
    ...
    >
        ...
        >
            ...
            >
                <div
                    class="tw-border-white/80 tw-bg-gray-100/30 tw-p-2 tw-border tw-font-semibold tw-leading-5"
                >
                    MEMO: {{ form.bill_no }}
                </div>
                <div class="tw-w-full">
                    <div class="tw-flex tw-w-full">
                        <ComboBox
                            ...
                            @leaving-combobox="focusElement('#search-product')"
                        />
                        <Link
                            as="button"
                            href="#"
                            class="tw-flex tw-justify-center tw-items-center tw-border-emerald-400 tw-bg-emerald-400 hover:tw-bg-emerald-500 tw-shadow tw-px-2.5 tw-border tw-w-12 tw-text-white tw-text-xl"
                        >
                            +
                        </Link>
                    </div>
                    ...
                    <div
                        ...
                        class="tw-bg-red-100 tw-mt-1 tw-pl-1 tw-border tw-border-red-400 tw-text-red-950"
                    >
                        {{ errors.customer_id }}
                    </div>
                </div>
            ...
            <!-- Second Column -->
            <div
                class="sm:tw-text-right tw-bg-amber-100/60 tw-p-2 sm:tw-w-1/3 tw-text-center"
            >
                <div
                    class="tw-inline-flex tw-items-baseline tw-font-normal tw-text-sm"
                >
                    <span class="tw-mr-2">{{ form.sale_date }}</span>
                    <span class="fa fa-calendar"></span>
                </div>
                <h2
                    class="sm:tw-text-right -tw-mt-1.5 tw-font-light tw-text-amber-900"
                >
                    Current Due
                    <span class="tw-font-bold tw-text-4xl"
                        ><br />{{ customer.total_due }}</span
                    >
                </h2>
            </div>
            ...
            >
                <template v-if="customer">
                    ...
                    <h3 class="tw-font-semibold tw-text-xl tw-truncate">
                        {{ customer.name }}
                    </h3>
                    <p>
        ...
        <div class="tw-flex tw-items-center tw-gap-x-3">
            <div class="tw-flex tw-items-center tw-w-2/3">
                <ProductSearch
                    @product-selected="addProductToInvoice"
                    @goto-products-area="goToProductArea"
                />
                <Link
                    as="button"
                    href="#"
                ...
                <div v-if="errors.sales_items" class="tw-pl-2 tw-text-error">
                    {{ errors.sales_items }}
                </div>
            </div>
            <div
                class="tw-flex tw-justify-center tw-items-center tw-bg-amber-300 tw-h-12 tw-w-1/3"
            >
                STOCK: {{}}
            </div>
        </div>
        ...
        <!-- Testing new layout -->
        <div
            class="tw-gap-0.5 tw-grid tw-grid-cols-[32px_1fr_90px_90px_90px_90px_30px]"
        >
            <!-- Header (sticky) -->
            <div class="tw-h-8 tw-text-center __thead">SL</div>
            <div
                class="tw-justify-start tw-pl-3 tw-h-8 tw-min-w-28 tw-truncate __thead"
            >
                Product Name
            </div>
            ...
            >
                <template
                    v-for="(product, index) in form.sales_items"
                    :key="product.id"
                >
                    <div class="tw-h-8 __tbody">{{ index + 1 }}</div>
                    <div
                        class="tw-justify-start tw-pl-3 tw-h-8 tw-truncate tw-uppercase __tbody"
                    >
                        {{ product.title }}
                    </div>
                    <div class="tw-h-8 __tbody">
                        <CurrencyInput
                            classes="tw-input-sm tw-h-8"
                            v-model="product.quantity"
                            :id="`input-${index}-quantity`"
                            @keydown.up.prevent="
                                navigateTo(index - 1, 'quantity')
                            "
                    ...
                    <div class="tw-h-8 __tbody">
                        <CurrencyInput
                            ...
                            :id="`input-${index}-rate`"
                            @keydown.up.prevent="navigateTo(index - 1, 'rate')"
                            @keydown.down.prevent="
                                navigateTo(index + 1, 'rate')
                            "
                            @keydown.left.prevent="
                                navigateTo(index, 'quantity')
                            "
                            @keydown.right.prevent="
                                navigateTo(index, 'discount')
                            "
                    ...
                    <div class="tw-h-8 __tbody">
                        <CurrencyInput
                            ...
                            :id="`input-${index}-discount`"
                            @keydown.up.prevent="
                                navigateTo(index - 1, 'discount')
                            "
                            @keydown.down.prevent="
                                navigateTo(index + 1, 'discount')
                            "
                    ...
                    <div class="tw-h-8 __tbody">
                        ...
                        >
                            <Icon
                                icon="delete"
                                class="tw-size-4 tw-stroke-red-950"
                            />
                        </button>
                    </div>
                </template>
            ...
            >
                ...
                >
                    ...
                    <div
                        v-if="form.payment_type?.ref_name"
                        class="tw-form-control"
                    >
                        <label class="tw-mb-0 tw-pt-0 tw-label">
                            <span
                                class="tw-label-text tw-font-normal tw-text-sm"
                                >{{ form.payment_type?.ref_name }}</span
                            >
                        </label>
                        <input
                            type="text"
                ...
                <div
                    class="tw-col-span-full md:tw-col-span-2 tw-row-span-4 tw-p-2"
                >
            ...
            <div
                class="tw-gap-0.5 tw-grid tw-grid-cols-subgrid tw-col-span-4 tw-row-span-6"
            >
                <!-- Total -->
                ...
                <div
                    class="tw-flex tw-justify-center tw-items-center tw-bg-gray-100 tw-h-8 tw-text-xs"
                >
                    BDT
                </div>
                ...
                <div class="__tfoot_sum_amount">
                    <CurrencyInput
                        ...
                        "
                        @keydown.down.prevent="
                            focusElement('#paid-amount input')
                        "
                        @keydown.enter.prevent="
                            focusElement('#paid-amount input')
                        "
                ...
                <div class="__tfoot_sum_amount">
                    <CurrencyInput
                        v-model.number="form.due_amount"
                        :disabled="true"
                        classes="tw-h-8 tw-content-center tw-font-black"
                    />
                </div>
        ...
        <div class="tw-gap-1 tw-grid tw-grid-cols-[1fr_310px] tw-mt-3">
            <div
                class="xl:tw-flex tw-flex-wrap tw-items-center tw-gap-x-5 tw-hidden"
            >
                <div>Search: <kbd>/</kbd></div>
                <div>Discount: <kbd class="">shift</kbd> + <kbd>/</kbd></div>
                <div>Save: <kbd>ctrl</kbd> + <kbd>Enter</kbd></div>
            </div>
            ...
            >
                ...
                <button
                    :disabled="form.processing || !form.sales_items.length"
                    id="save-button"
                    type="button"
                    @click.prevent="saveInvoice"
                    @keydown.up.prevent="focusElement('#paid-amount input')"
...
import { useInvoiceForm, focusElement } from '@/Composables/useInvoiceForm';
...
const form = useForm({
    ...
    sale_date: dayjs().format('MMM DD, YYYY'),
    customer: null,
    customer_id: null,
...
// Use the invoice form composable
const saveInvoice = () => {
    if (!form.processing) {
...
const { navigateTo, addProductToInvoice, removeItem, goToProductArea } =
    useInvoiceForm(form, {
        itemsKey: 'sales_items',
        hasPayment: true,
        onSave: saveInvoice,
        priceField: 'sales_price',
    });

// Customer-specific watchers
const customer = ref('');

const changeCustomer = (id) => {
    customer.value = props.customers.find((item) => item.id === id) || '';
};

watch(() => form.customer_id, changeCustomer);
watch(
    () => form.payment_type,
    (type) => {
