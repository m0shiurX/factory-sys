...
<template>
    ...
    <div class="tw-gap-2 tw-grid tw-grid-cols-4 tw-mx-auto tw-pb-12 tw-max-w-7xl">
        <!-- Navigation -->
        <div
        ...
        <div class="tw-col-span-4 tw-bg-base-100 tw-p-3 tw-border tw-rounded-sm">
            <form @submit.prevent="saveItem">
                ...
                <div
                    ...
                    <div class="tw-col-span-1">
                        ...
                        <div class="tw-w-full tw-max-w-sm">
                            <v-select class="tw-bg-white tw-rounded-sm" placeholder="Choose a Customer"
                                v-model.number="form.customer_id" :options="props.customers"
                                :reduce="(customer) => customer.id" label="name" />
                            <div class="tw-text-error" v-if="errors.customer_id">
                                {{ errors.customer_id }}
                            </div>
                        </div>
                    </div>
                    <div class="tw-col-span-2 tw-text-center">
                        <template v-if="customer !== ''">
                            <h3 class="tw-font-semibold tw-text-xl">{{ customer.name }}</h3>
                            <p>
                                {{ customer.address }} <br />
                    ...
                    <div class="tw-col-span-1 tw-col-start-4 print:tw-text-black">
                        <div class="tw-mt-0">
                            <label class="tw-mb-0 tw-font-semibold"> Invoice Date: </label>
                            <Datepicker v-model="form.sale_date" id="sale_date" />
                        </div>
                    </div>
                </div>
                ...
                <div class="tw-relative tw-flex-1 print:tw-hidden tw-mt-5">
                    <div>
                        <label
                            ...
                            <input name="search" v-model="search" placeholder="Type product name or code" type="text"
                                autocomplete="off" class="tw-rounded-none tw-w-full placeholder:tw-text-accent"
                            ...
                            <div v-if="search.length > 0" class="tw-cursor-pointer" @click="resetSearch">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="tw-size-5 tw-stroke-danger">
                    ...
                    <transition name="fade">
                        <div v-if="search.length > 0 && searchResultShown"
                            ...
                            <ul class="tw-bg-primary tw-overflow-auto">
                                <li v-for="(product, index) in filteredProduct" :key="product.id"
                                    ...
                                    role="option" tabindex="-1" value="0" :class="{
                                        'tw-border-y tw-border-accent-content tw-bg-accent-content':
                                            index === highlightedIndex,
                                    }" @click="clickedItem(index)">
                                    <span class="tw-block tw-font-normal tw-capitalize">{{ product.title }} - {{
                                        product.unit }}</span>
                                </li>
                            </ul>
                        </div>
                ...
                <table class="tw-my-3 tw-table-auto tw-w-full">
                    <thead>
                        <tr
                            ...
                            <th class="tw-border-gray-400 tw-border-x tw-pr-2 tw-pl-5 tw-w-3 tw-text-left">
                                SL
                            </th>
                            <th class="tw-border-gray-400 tw-pr-2 tw-pl-5 tw-border-r tw-w-60 tw-text-left">
                                Product Name
                            </th>
                            <th class="tw-border-gray-400 tw-border-x tw-pl-5 tw-w-28 tw-text-center">
                                Rate
                            </th>
                            <th class="tw-border-gray-400 tw-border-x tw-pl-5 tw-w-28 tw-text-center">
                                Quantity
                            ...
                            <th class="tw-border-gray-400 tw-border-x tw-pl-5 tw-w-32 tw-text-center">
                                Total
                            </th>
                            <th class="tw-border-gray-400 tw-border-x tw-pr-3 tw-pl-5 tw-w-8 tw-text-center">
                                ACT
                            </th>
                        </tr>
                        <tr class="tw-h-2"></tr>
                    ...
                    <tbody>
                        <template v-for="(formRow, index) in form.sales_items" :key="formRow.id">
                            <tr
                                ...
                                <td class="tw-border-gray-300 tw-border-x tw-pr-2 tw-pl-5 tw-w-3 tw-text-left">
                                    {{ index + 1 }}
                                </td>
                                <td class="tw-border-gray-300 tw-pr-2 tw-pl-5 tw-border-r tw-text-left">
                                    {{ formRow.title }} - {{ formRow.unit }}
                                </td>
                                <td class="tw-border-gray-300 tw-border-x tw-w-32 tw-text-center">
                                    <CurrencyInput v-model="formRow.unit_price" :disabled="false" :step="1" />
                                </td>
                                <td class="tw-border-gray-300 tw-border-x tw-w-32 tw-text-center">
                                    <CurrencyInput v-model="formRow.quantity" :disabled="false" />
                                </td>
                                <td class="tw-border-gray-300 tw-border-x tw-w-32 tw-text-center">
                                    <CurrencyInput v-model="formRow.discount" :disabled="false" />
                                </td>
                                <td class="tw-border-gray-300 tw-border-x tw-w-32 tw-text-center">
                                    <CurrencyInput v-model="formRow.sub_total" :disabled="true" :step="1" />
                                </td>
                                <td>
                ...
                <div class="tw-gap-x-12 tw-grid tw-grid-cols-6 tw-p-4">
                    <div class="tw-space-y-3 tw-col-span-6 sm:tw-col-span-3 xl:tw-col-span-4">
                        <!-- Invoice Notes -->
                        <label class="tw-form-control" v-if="form.extra_fees > 0">
                            <div class="tw-label">
                                <span class="tw-label-text tw-font-semibold">Extra fees details</span>
                            </div>
                            <textarea v-model="form.sale_note" class="tw-textarea-bordered tw-textarea h-24"
                                placeholder="Max. 300 characters"></textarea>
                        </label>
                    </div>

                    <!-- Invoice Summary -->
                    <div class="tw-col-span-6 sm:tw-col-span-3 xl:tw-col-span-2">
                        <div class="tw-flex tw-flex-col tw-h-full">
                            <div class="tw-space-y-2 tw-mb-6">
                                <div class="tw-flex tw-justify-between tw-items-center">
                                    <p class="tw-grow tw-font-semibold">Subtotal ৳</p>
                                    <p class="tw-w-28">
                                        <CurrencyInput classes="tw-input-bordered tw-input-sm"
                                            v-model.number="form.sub_total" :disabled="true" :step="1" />
                                    </p>
                                </div>
                                <div class="tw-flex tw-justify-between tw-items-center">
                                    <p class="tw-grow tw-font-semibold">Discount</p>
                                    <p class="tw-w-28">
                                        <CurrencyInput classes="tw-input-bordered tw-input-sm"
                                            v-model.number="form.discount" :disabled="false" :step="1" />
                                    </p>
                                </div>
                                <div class="tw-flex tw-justify-between tw-items-center">
                                    <p class="tw-grow tw-font-semibold">Extra Fees</p>
                                    <p class="tw-w-28">
                                        <CurrencyInput classes="tw-input-bordered tw-input-sm"
                                            v-model.number="form.extra_fees" :disabled="false" :step="1" />
                                    </p>
                                </div>
                                <div class="tw-flex tw-justify-between tw-items-center tw-pt-3 tw-border-t">
                                    <p class="tw-grow tw-font-bold">Grand Total</p>
                                    <p class="tw-w-28">
                                        <CurrencyInput classes="tw-input-bordered tw-input-sm"
                                            v-model.number="form.grand_total" :disabled="true" :step="1" />
                                    </p>
                                </div>
                            </div>
                            <div class="tw-mt-auto">
                                <div class="tw-flex tw-justify-between tw-items-center tw-gap-x-3 tw-pt-6">
                                    <Link :href="route('admin.sales.index')" as="button" tabindex="0"
...
watch(
    ...
    (items) => {
        items.forEach((item) => {
            ...
            // Calculate subtotal
            const subTotal =
                parseFloat(item.unit_price) * item.quantity * (1 - discountDecimal);
...
form.sub_total = computed(() => {
    return parseFloat(
        form.sales_items
            .reduce((accumulator, current) => accumulator + parseFloat(current.sub_total), 0)
            .toFixed(2)
    );
});
